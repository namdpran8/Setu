...
static ssize_t sys_pread(borrowed_fd fd, void* data, size_t byte_count, off64_t offset) {
  DWORD bytes_read;
...
static ssize_t sys_pwrite(borrowed_fd fd, const void* data, size_t byte_count, off64_t offset) {
  DWORD bytes_written;
  OVERLAPPED overlapped;
  memset(&overlapped, 0, sizeof(OVERLAPPED));
  overlapped.Offset = static_cast<DWORD>(offset);
...
bool ReadFullyAtOffset(borrowed_fd fd, void* data, size_t byte_count, off64_t offset) {
  uint8_t* p = reinterpret_cast<uint8_t*>(data);
  while (byte_count > 0) {
    ssize_t n = TEMP_FAILURE_RETRY(sys_pread(fd.get(), p, byte_count, offset));
    if (n == 0) {  // EOF
      errno = ENODATA;
      return false;
    }
...
bool WriteFullyAtOffset(borrowed_fd fd, const void* data, size_t byte_count, off64_t offset) {
  ...
  size_t remaining = byte_count;
  while (remaining > 0) {
    ssize_t n = TEMP_FAILURE_RETRY(sys_pwrite(fd.get(), p, remaining, offset));
    if (n == -1) return false;
    p += n;
    remaining -= n;
    offset += n;
...
std::string Dirname(std::string_view path) {
  ...
  if (last_slash != std::string_view::npos) {
    ...
    if (last_slash == 0) return std::string(path.substr(0, 1));
